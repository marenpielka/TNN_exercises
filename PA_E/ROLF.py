...
class ROLF:
    ...
    '''
    def __init__(self, eta_sigma, eta_c, init_sigma, init_method, rho=2.0, max_num_neurons=10000, max_num_patterns=100000,\
                 max_input_dim=10):
    ...
    def train(self, patterns):
        # check für consistency with input_dim
        ...
        for pattern in patterns:
            ...
            for neuron in self.neurons:
                ...
                if self.dist(pattern, neuron.center_pos) <= neuron.sigma*self.rho:    
                    ...
                    if accepting_neuron == None or self.dist(pattern, neuron.center_pos) < self.dist(pattern, accepting_neuron.center_pos):
                        accepting_neuron = neuron
                        
            if accepting_neuron != None:
...
def main():
    ...
    with open('training3.dat', 'r') as fp:
        lines = list(fp)
    ...
    with open("output3.txt", "w") as fp:
        fp.write("Results for initial sigma = {0}, eta_c = {1}, eta_sigma = {2}, \"{3}\" method\n\n".format(init_sigma,\
                 eta_sigma, eta_c, init_method))
        fp.write("Neuron center positions{0}Neuron sizes\n".format("".join(["\t" for i in range(in_size-2)])))
